...
const Create = () => {
    const { user } = useGlobalContext() as any
            ...
            <ScrollView className='px-4 my-6'>
                ...
                <View className='mt-7 space-y-2'>
                    ...
                    <TouchableOpacity onPress={() => openPicker('video')}>
                        {form.video ? (
                            <Video source={{ uri: (form.video as any)?.uri }}
                ...
                <View className='mt-7 space-y-2'>
                    ...
                    <TouchableOpacity onPress={() => openPicker('image')}>
                        {form.thumbnail ? (
                            <Image source={{ uri: (form.thumbnail as any)?.uri }}
                                className='w-full h-64 rounded-2xl'
                                resizeMode='cover'
                            />
                        ) : (
                            <View className='w-full h-16 px-4 bg-black-100 rounded-2xl justify-center items-center border-2 border-black-200 flex-row space-x-2'>
                                <Image source={icons.upload}
                                    ...
                                    className='w-5 h-5'
                                />
                                <Text className='text-sm text-gray-100 font-pmedium'>Select a file</Text>
                            </View>
                        )}
                    </TouchableOpacity>
                </View>
                <FormField
                    title='Prompt'
                    value={form.prompt}
                    placeholder='Prompt to create a video'
